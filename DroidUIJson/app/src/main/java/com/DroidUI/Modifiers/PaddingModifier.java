...
public class PaddingModifier implements ViewModifier {
    ...
    PaddingModifier() { this(Edge.Set.all, defaultPadding); }
    PaddingModifier(Edge.Set edges) { this(edges, defaultPadding); }
    //PaddingModifier(float length) { this(Edge.Set.all, length); }
    PaddingModifier(EnumSet<Edge> edges, float length) {
        this.value = new EdgeInsets(edges.contains(Edge.top) ? length : 0,
                                edges.contains(Edge.leading) ? length : 0,
                                edges.contains(Edge.bottom) ? length : 0,
                                edges.contains(Edge.trailing) ? length : 0);
    }
